...
export default function Profile() {
    ...
    const getUser = async () => {
        setLoading(true);
        try {
            let response = await axios.get(`https://backend-ic7841.herokuapp.com/api/private/users/${id}`, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("authToken")}`,
                },
    ...
    const handleSubmit = async e => {
        ...
        if (validate()) {
            ...
            try {
                await axios
                    .patch(`https://backend-ic7841.herokuapp.com/api/private/users/${id}`, values, config)
                    .then(confirmPost)
